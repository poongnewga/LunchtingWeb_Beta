    ...
    @fgroups.order("id").each do |f|
      ...
      @mgroups.where(people: f.people.to_i).each do |m|
        ...
        if !@days.nil?
          ...
          if 1.5 >= Geocoder::Calculations.distance_between([JSON.parse(m.location)["latitude"].to_f,JSON.parse(m.location)["longitude"].to_f], [JSON.parse(f.location)["latitude"].to_f,JSON.parse(f.location)["longitude"].to_f])
            puts "비교성공"
